let xenon = function (par) {
  ...
  getcsvdata(GoogleSheetCsvURL(jsonfile), function (dadosa) {
    let dados = select(dadosa, multipatterncheck_exclude, par);
    ...
    getcsvdata(GoogleSheetCsvURL(jsonfile2), function (ojsondata) {
      let jsondata = select(ojsondata, multipatterncheck_exclude, par);

      for (let i = 0; i < jsondata.length; i++) {
        xpto += `<a target='_blank' href='${jsondata[i].imagem}' class='rsslink'><div style='margin-bottom: 16px; padding-top: 100%; background-repeat: no-repeat; background-position: center center; background-size: cover; background-image: url(${jsondata[i].imagem});'></div>${jsondata[i].titulo} • ${jsondata[i].fonte}<br><br></a>`;
      }
